...
def calculate_alpha(t, T1, T2, syn_w): 
    return (syn_w/(T2-T1))*np.heaviside(t, 0.5)*(np.exp(-t/T2)-np.exp(-t/T1)) # post-synaptic conductance waveform
...
def get_ia_afferent_synaptic_potential(sim_dur, 
    ...
    file_path = f"data/synaptic_potentials/synaptic_potential_SCS_freq_{scs_freq}_n_axons_200_tx_{Tx}_tw_{Tw}_uw_{Uw}_ta_{Ta}_p_success_{p_axonal_success}_dt_{dt}_simulation_duration_2000.pickle"
    ...
    for t in range(t_vec_len): 
        for j in range(n_axons): 
            if (scs_pulses[t] == 1):      
                ...
                if (random.random() < p_axonal_success) & (axon_activation[j][t_spike] == 1): # if spike succeeds (axonal success)
                    activation_delay = int(Ta + np.random.randn()*0.2*Ta)
                    axon_activation[j][t_spike:t_spike+activation_delay] = 0
                    if (random.random() < p[j][t_spike]): # if spike success (synaptic success)
...
if __name__ == "__main__":
    ...
    plt.figure(1)
    #test = get_ia_afferent_synaptic_potential(200,80,500, p_axonal_success=0.2)
    np.random.seed(1)
    # max
    release_control_ux = get_ia_afferent_release_prob(time, hz, n_axons, p_axonal_success=0.5, Ta=0, Tw=20, delay=0)
    ...
    # rest
    release_high_ux = get_ia_afferent_release_prob(time, hz, n_axons, p_axonal_success=0.3, Ta=0, Tw=80, delay=0)
